...
Q = np.diag([500.0, 500.0, 20.0, 1.0])  # x, y, yaw, v
...
MAX_V = 5.0 # m/s
MAX_OMEGA = np.deg2rad(120) # rad/s
MAX_DOMEGA = 5.0
MAX_ACCEL = 5.0  # m/s^2
...
class State:
    ...
    """
    def __init__(self, x=0.0, y=0.0, yaw=0.0, v=0.0): #start vehicle at 0s for everything
        self.x = x
        self.y = y
        self.yaw = yaw
        self.v = v
...
def plot_car(x, y, yaw, size=0.25, color='b'):
    """
    Description:
...
def linear_mpc_control(xref, xbar, x0, other_predicted_trajs=None, formation_constraints=None):
    ...
    FORMATION_WEIGHT = 1000.0  # Strong penalty to preserve shape
    ...
    for t in range(T):
        ...
        # Optional: velocity deviation from nominal target
        # Dynamic speed matching: if tracking a formation constraint, try to match that robot’s velocity
        if formation_constraints:
            # Estimate follower target velocity based on offset direction
            target_speed = xref[3, t]

            # Encourage speeding up or slowing down relative to offset direction
            # if you're behind, speed up — if you're ahead, slow down
            for pred_x, pred_y, dx_target, dy_target in formation_constraints:
                ...
                speed_adjust = -1.0 * dist_error  # negative if too close, positive if lagging
                target_speed += speed_adjust * 1.5  # tune this gain
                offset_mag = math.hypot(dx_target, dy_target)
                target_speed += 0.5 * offset_mag  # tune this gain

            v_error = x[3, t] - target_speed
            cost += 10.0 * cvxpy.square(v_error)
        else:
            v_error = x[3, t] - xref[3, t]
            cost += 5.0 * cvxpy.square(v_error)
        ...
        if formation_constraints:
            ...
            for pred_x, pred_y, dx_target, dy_target in formation_constraints:
                ...
                # Rotate target offset into global frame
                # --- Curvature-aware scaling ---
                kappa = 0.0
                if t < len(xref[2, :]) - 1:
                    dyaw = xref[2, t + 1] - xref[2, t]
                    ds = xref[3, t] * DT + 1e-3  # prevent div-by-zero
                    kappa = dyaw / ds

                # Curvature-aware scaling of formation shape
                scale_x = 0.3  # tune this
                scale_y = 0.5  # tune this
                ...
                heading_error = x[2, t] - yaw  # same yaw as leader at t
                cost += 500.0 * cvxpy.square(heading_error)
                cost += FORMATION_WEIGHT * cvxpy.square(rel_x_error)   # forward direction
                cost += (FORMATION_WEIGHT * 3.0) * cvxpy.square(rel_y_error)  # lateral direction (double penalty)



    for t in range(T - 2):
        jerk = u[0, t + 2] - 2 * u[0, t + 1] + u[0, t]
        cost += 500.0 * cvxpy.square(jerk)
...
def calc_ref_trajectory(state, cx, cy, cyaw, ck, sp, dl, pind, unlock_index_limit=None):
    """
    Generates a velocity-based reference trajectory.
    Applies optional clamping via unlock_index_limit to prevent overtaking.
    """
    ...
    for i in range(T + 1):
        ...
        if index < ncourse:
            ...
            xref[1, i] = cy[index]
            xref[2, i] = cyaw[index]
            xref[3, i] = sp[index] if index < ncourse - 5 else 0.0
        else:
            xref[0, i] = cx[-1]
            xref[1, i] = cy[-1]
            xref[2, i] = cyaw[-1]
...
def run_formation_simulation(cx, cy, cyaw, ck, sp_center, dl, robot_states, formation_offsets):
    ...
    offset_distance = 0.25  # lateral separation

    while time <= MAX_TIME:
        for i in range(num_robots):
            state = robot_states[i]
            offset = formation_offsets[i]
            lateral_sign = np.sign(formation_offsets[i][1])  # +1 for left, -1 for right
            cx_i, cy_i = get_offset_path(cx, cy, cyaw, lateral_sign * offset_distance)

            # Compute offset-adjusted curvature
            offset_mag = np.linalg.norm(offset)
            ck_i = [k / (1 - k * offset_mag) if abs(1 - k * offset_mag) > 1e-3 else k for k in ck]

            # Compute custom speed profile
            sp_i = calc_speed_profile(cx_i, cy_i, cyaw, ck_i, TARGET_SPEED)

            # Apply gating delay for followers
            unlock_limit = None
            if i != 0:
                unlock_limit = max(0, target_inds[0] - 4)
                if i in [2, 3]:
                    kappa = ck[target_inds[0]] if target_inds[0] < len(ck) else 0.0
            ...
            if i in [1, 2, 3]:
                # Define leader: R1 follows R0, R2 follows R0, R3 follows R1
                leader_idx = 0 if i in [1, 2] else 1
                ...
                formation_constraint = None  # R0 has no leader

            oa, od, ox, oy, oyaw, ov = iterative_linear_mpc_control(
                xref, x0, oa_list[i], od_list[i],
            ...
            robot_states[i] = update_state(state, oa[0], od[0])
        ...
        if show_animation:
            ...
            # Plot offset paths (just 2: left and right)
